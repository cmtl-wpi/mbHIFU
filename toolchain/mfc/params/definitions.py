...
def _load():  # pylint: disable=too-many-locals,too-many-statements
    ...
    for a, sym in [("R0ref", r"\f$R_0\f$"), ("p0ref", r"\f$p_0\f$"),
                   ...
                   ("ss0", r"\f$\sigma_0\f$"), ("dil_vsc", r"\f$dil_vsc\f$"),
                   ("el_vsc", r"\f$el_vsc\f$"), ("Thost", r"\f$T_{host}\f$")]:
    ...
    # --- hifu_params ---
    for a in ["sampling", "heatSolver", "intPrms", "streaming", "automatic_stages",
              "stg1", "stg2", "stg3", "stg3_3d", "cartesian", "moments","power_balance"]:
        _r(f"hifu_params%{a}", LOG, {"hifu"})
    for a in ["stepStopSource", "t_step_stop_stg1", "t_step_stop_stg2", "t_step_stop_stg3",
              "t_step_save_stg3", "p_cyl", "m", "n", "p"]:
        _r(f"hifu_params%{a}", INT, {"hifu"})
    for a in ["Tref", "K", "alpha", "atmPres", "absCoef", "t_stop_stg1", "t_stop_stg2",
