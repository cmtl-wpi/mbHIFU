...
contains
    ...
    !! @param particleTime Current time (Runge-Kutta time stepper)
    subroutine s_acoustic_src_calculations(q_cons_vf, q_prim_vf, t_step, rhs_vf, particleTime)
        ...
        !! Adjust current time to make it compatible with the Runge-Kutta time stepper
        !! which is used in the lagrangian subgrid bubble model.
        if (present(particleTime)) then
            sim_time = particleTime
        else
            sim_time = t_step*dt
        end if
    ...
    subroutine s_source_spatial_transducer(ai, sig, r, source, angle, xyz_to_r_ratios)
        ...
        real(kind(0d0)) :: current_angle, angle_half_aperture, dist, dist2, norm
