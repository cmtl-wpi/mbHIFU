    ...
    subroutine s_time_stepper_heatEqn(t_step)
        ...
        if (hifu_params%cartesian) then
            ...
            if (proc_rank == 0) then
                ...
                $:GPU_PARALLEL_LOOP(collapse=3,reduction='[[temp_max],[temp_min]]',reductionOp='[MAX, MIN]',copy='[temp_max, temp_min]')
                do l = 0, p_hf
                    do k = 0, n_hf
                        do j = 0, m_hf
            ...
            if (hifu_params%stg3_3d) then   ! Cylindrical coord
                ...
                $:GPU_PARALLEL_LOOP(collapse=3,reduction='[[temp_max],[temp_min]]',reductionOp='[MAX, MIN]',copy='[temp_max, temp_min]', copyin='[t_step]')
                do l = 0, p
