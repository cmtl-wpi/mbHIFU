    ...
    impure subroutine s_save_data(t_step, start, finish, io_time_avg, nt)
            ...
            do l = 0, p
                do k = 0, n
                    do j = 0, m
                        ...
                        if (ieee_is_nan(q_cons_ts(1)%vf(i)%sf(j, k, l_mod))) then
        ...
        if (.not. hifu_params%heatSolver) then
            if (bubbles_lagrange) then
                ...
                call s_write_data_files(q_cons_ts(1)%vf, q_T_sf, q_prim_vf, save_count, &
                                                                    bc_type, q_beta%vf(1))
                $:GPU_UPDATE(host='[intfc_vel,Rmax_stats,Rmin_stats,gas_p,gas_mv,mrmtnt_shell,bub_qvis,bub_qth,intfc_ac,bub_hifu_rad]')
