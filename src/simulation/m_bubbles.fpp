...
contains
    ...
    function f_cpbw_KM(fR0, fR, fV, fpb, fshell, fRbuck)
        !$acc routine seq
        real(kind(0d0)), intent(in) :: fR0, fR, fV, fpb
        real(kind(0d0)), intent(in), optional :: fshell, fRbuck
        ...
        if (lag_params%coatedBub_model) then
            if (fshell==1d0) then
                if (fR <= fRbuck) then  !Buckling regime
                    f_cpbw_KM = f_cpbw_KM - 4d0*lag_params%srfDilVsc_ctdBub*fV/(fR**2d0)
                else                    !Elastic regime
                    f_cpbw_KM = f_cpbw_KM - 2.0d0*(lag_params%srfElast_ctdBub * ((fR/fRbuck)**2d0 - 1d0))/fR &
                                                                - 4d0*lag_params%srfDilVsc_ctdBub*fV/(fR**2d0)
                end if
            elseif (fshell==0d0) then   !Rupture regime
                f_cpbw_KM = f_cpbw_KM - 2.d0/(fR*Web)
            end if
        else
    ...
    function f_vflux(fR, fV, fmass_v, iR0, fmass_n, fbeta_c, fconc_v, fshell)
        !$acc routine seq
        real(kind(0.d0)), intent(in) :: fR
        real(kind(0.d0)), intent(in) :: fV
        real(kind(0.d0)), intent(in) :: fmass_v
        integer, intent(in) :: iR0
        real(kind(0.d0)), intent(in), optional :: fmass_n, fbeta_c, fconc_v, fshell
        ...
        if (thermal == 3) then !transfer
            ...
            if (bubbles_lagrange) then
                ...
                f_vflux = 0d0
                if (lag_params%massTransfer_model .and. (fshell==0d0)) then
    ...
    !>  Function that computes the time derivative of
        ...
        !!  @param fshell Shell indicator (EL)
    function f_bpres_dot(fvflux, fR, fV, fpb, fmass_v, iR0, fbeta_t, fR_m, fgamma_m, fshell)
        ...
        real(kind(0.d0)), intent(in) :: fmass_v
        integer, intent(in) :: iR0
        real(kind(0.d0)), intent(in), optional :: fbeta_t, fR_m, fgamma_m, fshell
        ...
        if (thermal == 3) then
            if (bubbles_lagrange) then
                T_bar = fpb*(4d0/3d0*pi*fR**3d0)/fR_m
                grad_T = -fbeta_t*(T_bar - Tw)
                heatflux = 0d0
                if (lag_params%heatTransfer_model .and. (fshell==0d0)) then
                    heatflux = (fgamma_m - 1d0)/fgamma_m*grad_T/fR
                end if
                f_bpres_dot = 3d0*fgamma_m*(-fV*fpb + fvflux*R_v*Tw &
                                             + heatflux)/fR
                return
            end if
