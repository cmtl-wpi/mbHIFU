...
contains
    ...
    elemental function f_rddot(fRho, fP, fR, fV, fR0, fpb, fpbdot, alf, fntait, fBtait, f_bub_adv_src, f_divu, fCson, fInt, fshell, fRbuck, fRcell)
    ...
    elemental function f_rddot_KM(fpbdot, fCp, fCpbw, fRho, fR, fV, fR0, fC, fInt, fshell, fRbuck, fpb)
        ...
        if (bubbles_lagrange .and. fshell == 1._wp) then
            ...
            cdot_star = cdot_star + 2._wp*ss_mod*fV/(fR**2._wp) + &
                        8._wp*dil_vsc*(fV**2._wp)/fR**3._wp
        else
    ...
    elemental subroutine s_vflux(fR, fV, fpb, fmass_v, iR0, vflux, fmass_g, fbeta_c, fR_m, fgamma_m, fshell)
        ...
        real(wp), intent(in), optional :: fmass_g, fbeta_c, fshell
        ...
        do
            ...
            do while (iter_count < adap_dt_max_iters)
                ...
                call s_advance_substep(err(1), &
                                       ...
                                       fntait, fBtait, f_bub_adv_src, f_divu, &
                                       bub_id, fmass_v, fmass_g, fbeta_c, fbeta_t, &
                ...
                call s_advance_substep(err(3), &
                                       ...
                                       fntait, fBtait, f_bub_adv_src, f_divu, &
                                       bub_id, fmass_v2, fmass_g, fbeta_c, fbeta_t, &
                ...
                if ((err(1) <= adap_dt_tol) .and. (err(2) <= adap_dt_tol) .and. &
                    ...
                    !     print *, 'otherVars', fmass_g, fbeta_c, fbeta_t, fCson, fshell, fRbuck, fRrupt
                    ! end if
    ...
    subroutine s_advance_substep(err, fRho, fP, fR, fV, fR0, fpb, fpbdot, alf, &
        ...
        integer, intent(IN) :: bub_id
        real(wp), intent(out) :: fAc
        real(wp), intent(IN) :: fmass_v, fmass_g, fbeta_c, fbeta_t, fCson, fshell, fRbuck, fInt, fRcell
        real(wp), intent(in) :: fnoise_constant, flambda_c, fdk, floc, ftime
        ...
        if (bubbles_lagrange) then
            ...
            call s_advance_EL(myR_tmp(1), myV_tmp(1), myPb_tmp(1), myMv_tmp(1), bub_id, &
                              fmass_g, fbeta_c, fbeta_t, mydPbdt_tmp(1), mydMvdt_tmp(1), fshell)
        end if
        ...
        if (bubbles_lagrange) then
            ...
            call s_advance_EL(myR_tmp(2), myV_tmp(2), myPb_tmp(2), myMv_tmp(2), &
                              bub_id, fmass_g, fbeta_c, fbeta_t, mydPbdt_tmp(2), mydMvdt_tmp(2), fshell)
        end if
        ...
        if (bubbles_lagrange) then
            ...
            call s_advance_EL(myR_tmp(3), myV_tmp(3), myPb_tmp(3), myMv_tmp(3), &
                              bub_id, fmass_g, fbeta_c, fbeta_t, mydPbdt_tmp(3), mydMvdt_tmp(3), fshell)
        end if
        ...
        if (bubbles_lagrange) then
            ...
            call s_advance_EL(myR_tmp(4), myV_tmp(4), myPb_tmp(4), myMv_tmp(4), &
                              bub_id, fmass_g, fbeta_c, fbeta_t, mydPbdt_tmp(4), mydMvdt_tmp(4), fshell)
        end if
    ...
    elemental subroutine s_advance_EL(fR_tmp, fV_tmp, fPb_tmp, fMv_tmp, bub_id, &
                                      fmass_g, fbeta_c, fbeta_t, fdPbdt_tmp, advance_EL, fshell)
        $:GPU_ROUTINE(parallelism='[seq]')
