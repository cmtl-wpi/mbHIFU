...
102             continue
    ...
    subroutine s_add_particle(inputparticle, q_cons_vp, q_prim_vf)
        ...
        particleinfo%mv = pvap*volparticle*(1/Rvap)*dble(massflag) ! vapermass
        particleinfo%mg = (particleinfo%p - pvap*dble(massflag))*volparticle*(1/Rgas) ! gasmass
        ...
        if (pvap*real(massflag) > particleinfo%p) then
        ...
        particleinfo%betaT = f_transfercoeff(PeT, 1.0d0)*real(heatflag)
        PeG = particleinfo%R0**2*omegaN/diffcoefvap
        particleinfo%betaC = f_transfercoeff(PeG, 1.0d0)*real(massflag)
        ...
        if (avgdensFlag) then
            ...
            if ((clusterflag > 0) .or. (correctpresFlag)) then ! subgrid p_inf model from kazuki and Colonius
                ...
                do while (associated(cellwbaux))
                    ...
                    do while (associated(particle))
                        pint = f_pressureliq_int(particle%data%tmp%p, particle%data%tmp%y(1), particle%data%tmp%y(2)) + &
                                                                                        0.5d0*particle%data%tmp%y(2)**2
    ...
    subroutine s_compute_RP(bubbletmp, step, q, q_prim, qtime)
        ...
        pliqint = f_pressureliq_int(pbubble, bubbletmp%tmp%y(1), bubbletmp%tmp%y(2)) ! pres outside the bubble on the surface
    ...
    subroutine s_compute_interface_fluxes(bubbletmp, vaporflux, heatflux, gammabubble)
        ...
        bubbleTemp = bubbletmp%mg*Rgas + bubbletmp%tmp%mv*Rvap
...
711     if (num_procs > 1) then
    ...
    subroutine s_get_equilibrium_radius(ntrial, x, tolx, tolf, particle, pliq, largestep)
        ...
        do k = 1, ntrial
            ...
            fvec(1) = 4.0d0*pi*x(1)*(pliq - pvap) - 3.0d0*particle%mg*(Rgas)/x(1)**2 + 8.0d0*pi*sigmabubble
    ...
    function f_pressureliq_int(pbubble, radius, bubblevel)

        real(kind(0.d0)) :: f_pressureliq_int, radius, bubblevel, pbubble

        f_pressureliq_int = pbubble - 2.0d0*sigmabubble/radius - 4.*vischost*bubblevel/radius
...
3003        if ((num_dims == 3) .and. (k < epsilonbaux(3))) then
        ...
        ! if ((f_pressure_inf > 10) .or. (f_pressure_inf < -10) .or. (ieee_is_nan(f_pressure_inf))) then
        !     print *, 'Extreme pinf value of', f_pressure_inf, 'charvol2', charvol2, &
    ...
    subroutine s_write_particles(qtime)
        ...
        do while (associated(particle))
            ...
            write (11, '(6X,f12.6,I24.8,8f24.8)') &
                    qtime, &
                    particle%data%id, &
