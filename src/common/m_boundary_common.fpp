...
contains
    ...
    impure subroutine s_populate_variables_buffers(bc_type, q_prim_vf, pb_in, mv_in)
        ...
        if (bc_x%beg >= 0) then
            ...
            $:GPU_PARALLEL_LOOP(collapse=2, copyin='[mytime]')
            do l = 0, p
                do k = 0, n
    ...
    subroutine s_acoustic_bc(q_prim_vf, bc_dir, bc_loc, k, l, timeNow)
        $:GPU_ROUTINE(function_name='s_acoustic_bc',parallelism='[seq]', &
            & cray_inline=True)
        type(scalar_field), dimension(sys_size), intent(inout) :: q_prim_vf
        integer, intent(in) :: bc_dir, bc_loc
        integer, intent(in) :: k, l
    ...
    subroutine s_axis_cylindrical_sector_hifu(q_prim_vf, bc_dir, bc_loc, k, l)
        $:GPU_ROUTINE(function_name='s_axis_cylindrical_sector_hifu', &
            & parallelism='[seq]', cray_inline=True)
        type(scalar_field), dimension(sys_size), intent(inout) :: q_prim_vf
        integer, intent(in) :: bc_dir, bc_loc
        integer, intent(in) :: k, l
